...
fn main() {
    ...
    let swift_triple = match target_arch.as_str() {
        ...
        other => panic!("mediaplayer-rs: unsupported target arch '{other}'"),
    };
    ...
    println!("cargo:rustc-link-lib=static=MediaPlayerBridge");
    ...
    if let Ok(output) = Command::new("xcode-select").arg("-p").output() {
        if output.status.success() {
            ...
            // Add as both a build-time search path and a runtime rpath.
