...
- VERIFIED means the symbol has a usable Rust entry point. The `MPMediaItemProperty*` keys are usable through `NowPlayingInfo::value`. The `MPMediaPlaylistProperty*` constants and `MPMediaPlaybackIsPreparedToPlayDidChangeNotification` are exposed by name only, because no macOS API consumes them, so they are counted as EXEMPT. COVERAGE_PCT is VERIFIED / (SDK_PUBLIC_SYMBOLS - EXEMPT).
...
| Symbol | Kind | Header | Reason | SDK attribute |
| --- | --- | --- | --- | --- |
| `MPNowPlayingSessionDelegate` | protocol | `MPNowPlayingSession.h` | Unavailable on macOS. `async_api::NowPlayingSessionEventKind` exists for cross-platform code, and `NowPlayingSessionStream::subscribe` returns `NotAvailable`. | `MP_UNAVAILABLE_BEGIN(watchos, macos)` |
